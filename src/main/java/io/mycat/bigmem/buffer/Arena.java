...
public abstract class Arena<T> {
    ...
    **/
    ...
    private void allocateHuge(BaseByteBuffer<T> buffer, int capacity) {
        ...
        if (lastChunkSize != 0) {
            ...
            buffer.init(chunkArray, handle.getHandle(), c.getOffsetByHandle(handle.getHandle()), lastChunkSize,
                    lastChunkNormalSize);
        } else {
    ...
    /**
    * 创建一个hugeChunk 不进入缓存池,并却初始化baseByteBuffer
    * zhangwy  2017年1月2日 下午8:59:37
    **/
    ...
    /**
    * 分配一个tiny ,small, normal大小的handle
    * zhangwy  2017年1月7日 上午8:13:26
    **/
    ...
    /**
    * 
    * zhangwy  2017年1月7日 上午8:31:06
    **/
    private synchronized Handle allocateNormalHandle(int capacity, int normalSize) {
        Handle handleObj = null;
        if ((handleObj = q050.allocateHandle(capacity, normalSize)) != null
                || (handleObj = q025.allocateHandle(capacity, normalSize)) != null
                || (handleObj = q000.allocateHandle(capacity, normalSize)) != null
                || (handleObj = qInit.allocateHandle(capacity, normalSize)) != null
                || (handleObj = q075.allocateHandle(capacity, normalSize)) != null
                || (handleObj = q100.allocateHandle(capacity, normalSize)) != null) {
            return handleObj;
        }
        // Add a new chunk.
        Chunk<T> c = newChunk();
    ...
    /**
    * 创建若干不缓存的chunk,然后通过lastElementEmpty判断最后一个是否要放空.
    * zhangwy  2017年1月7日 上午7:54:35
    **/
    @SuppressWarnings("unchecked")
    ...
    **/
    ...
    public synchronized String toString() {
        StringBuilder buf = new StringBuilder().append("Chunk(s) at 0~25%:").append(StringUtil.NEWLINE).append(qInit)
                .append(StringUtil.NEWLINE).append("Chunk(s) at 0~50%:").append(StringUtil.NEWLINE).append(q000)
                .append(StringUtil.NEWLINE).append("Chunk(s) at 25~75%:").append(StringUtil.NEWLINE).append(q025)
                .append(StringUtil.NEWLINE).append("Chunk(s) at 50~100%:").append(StringUtil.NEWLINE).append(q050)
                .append(StringUtil.NEWLINE).append("Chunk(s) at 75~100%:").append(StringUtil.NEWLINE).append(q075)
                .append(StringUtil.NEWLINE).append("Chunk(s) at 100%:").append(StringUtil.NEWLINE).append(q100)
